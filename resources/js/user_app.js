...
const frontendComponents = require.context('./components/frontend', true, /\.vue$/i); // eslint-disable-line no-undef
// const chartComponents = require.context('./components/charts', true, /\.vue$/i); // eslint-disable-line no-undef
frontendComponents.keys().map(key => Vue.component(key.split('/').pop().split('.')[0], frontendComponents(key).default));
